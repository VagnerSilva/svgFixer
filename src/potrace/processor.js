...
Processor.prototype = {
    ...
    adjustVertices: function (path) {
        ...
        for (i = 0; i < m; i++) {
            ...
            while (1) {
                ...
                if (det !== 0.0) {
                    w.x =
                        (-Q.at(0, 2) * Q.at(1, 1) + Q.at(1, 2) * Q.at(0, 1)) /
                        det;
                    w.y =
                        (Q.at(0, 2) * Q.at(1, 0) - Q.at(1, 2) * Q.at(0, 0)) /
                        det;
                    break;
                }
    ...
    opti_penalty: function (path, i, j, res, opttolerance, convc, areac) {
        ...
        for (k = k1; k != j; k = k1) {
            ...
            if (
                this.sign(
                    this.cprod(vertex[i], vertex[i1], vertex[k1], vertex[k2])
                ) != conv
            ) {
                return 1;
            }
        ...
        area -=
            this.dpara(vertex[0], curve.c[i * 3 + 2], curve.c[j * 3 + 2]) / 2;
        if (i >= j) {
            area += areac[m];
        }
        ...
        for (i = 0; i < m; i++) {
            ...
            if (curve.tag[i1] == 'CURVE') {
                ...
                area +=
                    ...
                    2;
                area +=
                    this.dpara(p0, curve.c[i * 3 + 2], curve.c[i1 * 3 + 2]) / 2;
            }
            areac[i + 1] = area;
        }
